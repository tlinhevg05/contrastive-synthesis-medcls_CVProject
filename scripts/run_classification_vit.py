...
def main():
    ...
    if cfg["pretraining_strategy"] not in {"none", "imagenet", "covidqu", "imagenet_covidqu", "covidqu_syn", "imagenet_covidqu_syn"}:
        raise ValueError(f"Unsupported ViT-S/16 strategy: {cfg['pretraining_strategy']}")
    ...
    if args.pretrained_checkpoint is not None:
        ...
        finetune["checkpoint_path"] = str(output_dir / "pretrain/checkpoints/best_dino_teacher.pth")
    ...
    model = build_model("vit_s16", model_init, num_classes=4)
    if finetune["init"] == "contrastive_checkpoint":
        ...
        load_vit_dino_checkpoint(model, checkpoint_path)
